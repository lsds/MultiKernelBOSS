...
bool CGPUProjectToAFProject::Match(const CExpression* expr) {
  DynamicRegistry* dynamicRegistry = DynamicRegistry::GetInstance();
  auto engineType = dynamicRegistry->GetEngineType("ArrayFire");
  return expr->Pop()->Eopid() == dynamicRegistry->GetOperatorId(engineType, "CPhysicalGPUProject");
}

RetTypeC2B<EmptyStruct> CGPUProjectToAFProject::Translate(
...
int CGPUProjectToAFProject::GetPriority() {
  return 0;
}
}  // namespace cexpressiontoboss::translation
